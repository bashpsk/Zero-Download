package io.bashpsk.zerodownload.core.data.worker
...
import io.bashpsk.zerodownload.core.common.extension.hasAtomicPath
...
class MoveWorker @AssistedInject constructor(
    ...
    private val notificationBuilder = emptyNotification
        ...
        .setSmallIcon(R.drawable.ic_file_move)
    ...
    override suspend fun doWork(): Result = withContext(context = Dispatchers.IO) {
        ...
        return@withContext try {
            ...
            val byteCount = DEFAULT_BUFFER_SIZE.toLong()
            ...
            val isAtomicPath = hasAtomicPath(
                sourcePath = sourceFile.path,
                destinationPath = destinationFile.parent
            )
            ...
            progressJob = launch(context = Dispatchers.IO) {
                ...
                while (isActive) {
                    ...
                    notificationBuilder
                        .setContentTitle("Moving $savedSize/$totalSize ($progress%)")
            ...
            when (isAtomicPath) {
                ...
                true -> SystemFileSystem.atomicMove(
                    source = sourcePath,
                    destination = destinationPath
                )

                false -> fileSystem.source(path = sourcePath).use { source ->

                    fileSystem.sink(path = destinationPath).use { sink ->

                        val buffer = Buffer()

                        while (source.readAtMostTo(sink = buffer, byteCount = byteCount) >= 0) {
            ...
            when {
                ...
                !isAtomicPath && sourceFile.exists() && destinationFile.exists() -> {
                    ...
                    notificationBuilder
                        .setContentTitle("Deleting Original File...")
                        .setStyle(NotificationCompat.BigTextStyle().bigText(sourceFile.name))
            ...
            notificationBuilder
                ...
                .setStyle(NotificationCompat.BigTextStyle().bigText(destinationFile.name))
                .setProgress(100, 0, true)
            ...
            val failedNotification = emptyNotification
                ...
                .setContentTitle("File Move Failed")
